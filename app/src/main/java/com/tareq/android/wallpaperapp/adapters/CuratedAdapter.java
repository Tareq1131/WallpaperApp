package com.tareq.android.wallpaperapp.adapters;
...
import com.tareq.android.wallpaperapp.R;
...
public class CuratedAdapter extends RecyclerView.Adapter<CuratedViewHolder>{
    ...
    public CuratedViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new CuratedViewHolder(LayoutInflater.
                from(context).inflate(R.layout.home_list,parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull CuratedViewHolder holder, int position) {

        Picasso.get().load(list.get(position).getSrc().getMedium()).
                placeholder(R.drawable.ic_baseline_image_24).into(holder.imageView_list);
